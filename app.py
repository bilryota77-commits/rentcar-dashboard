...
if st.button("📊 예산 흐름 및 전후 대조표 추출 (API 연동)", key="power_sync_btn", type="primary"):
    if isinstance(date_range, tuple) and len(date_range) == 2:
        ...
        if all_camps:
            ...
            for idx, camp in enumerate(all_camps):
                cid = camp.get("nccCampaignId")
                cname = camp.get("name")
                ctype = camp.get("campaignTp", camp.get("type", "WEB_SITE"))
                
                place_targets = ["플레이스", "플레", "매장", "지점", "가양", "마곡", "인천", "김포", "안산", "일산", "강남", "양천"]
                ad_type = "플레이스" if (str(ctype).upper() in ["PLACE", "LOCAL_AD"] or any(k in cname for k in place_targets)) else "파워링크"
                short_name = re.sub(r'\s*\(.*?\)', '', cname).replace("파워링크#", "").replace("플레이스#", "").strip()

                status_text.markdown(f"[{idx+1}/{total_camps}] {short_name} 성과 대조 중...")
                progress_bar.progress((idx + 1) / total_camps)
                
                pre_spend, pre_clk, pre_ctr, pre_cpc = fetch_period_stat_api(cid, pre_start_str, pre_end_str)
                post_spend, post_clk, post_ctr, post_cpc = fetch_period_stat_api(cid, post_start_str, post_end_str)
                
                if pre_spend > 0 or post_spend > 0:
                    active_power_camps.append(camp)
                    bot_records.append({
                        "캠페인명": short_name, "광고종류": ad_type,
                        "조정 전 비용": pre_spend, "클릭률_전": pre_ctr, "CPC_전": pre_cpc,
                        "조정 후 비용": post_spend, "클릭률_후": post_ctr, "CPC_후": post_cpc,
                        "클릭수_후": post_clk
                    })
            ...
            daily_flow = {}
            for d_idx, target_date in enumerate(date_list):
                status_text.markdown(f"{target_date} 일일 예산 소진 흐름 추적 중...")
                day_power, day_place = 0, 0
                for camp in active_power_camps:
                    cid = camp.get("nccCampaignId")
                    cname = camp.get("name")
                    ad_type = "플레이스" if any(k in cname for k in ["플레이스", "매장", "지점"]) else "파워링크"
                    sp, clk, ctr, cpc = fetch_period_stat_api(cid, target_date, target_date)
                    if ad_type == "플레이스": day_place += sp
                    else: day_power += sp
                
                daily_flow[target_date] = {"파워링크": day_power, "플레이스": day_place, "일일소비금액": day_power + day_place}
