...
# ==========================================
# 글로벌 세션 상태 초기화 (💡 KeyError 완벽 차단 구역)
# ==========================================
...
# 💡 깜빡임 방지 및 KeyError 해결을 위해, 화면 렌더링 전 변수들을 맨 위에서 미리 선언합니다!
...
if st.button("📊 파워링크 통계 추출", key="power_sync_btn", type="primary"):
    if isinstance(date_range, tuple) and len(date_range) == 2:
        with st.spinner("파워링크 데이터를 스캔합니다..."):
            ...
            if err == "API_LIMIT_EXCEEDED":
                st.error("🚨 네이버 검색광고 API 호출 한도가 초과되었습니다.")
            elif all_camps:
